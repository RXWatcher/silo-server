...
- **Dedicated transcode-node restart is unrecovered (P-3 / tr-10).** The node
  404s on lost in-memory state and full-wipes its segment dir on boot. **Carried
  limitation** (accepted loss); TR-lease could fix it by handing the node the
  token, deferred.
...
- **Node-side reconstruct (P-3).** Give a dedicated node the token so it can
  self-reconstruct after its own restart.
...
- multi-node restart: the integrated front-end reconstructs only the `Session`
  ...
  transcode node itself reboots, the session is lost (accepted limitation, P-3).
