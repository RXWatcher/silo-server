...
func TestHandleBrowseItems_BatchDetailMatchesPerItem(t *testing.T) {
	...
	// per-item fetches; the failBatch run must fall back to per-item fetches.
	...
	if perItemContent.perItemCalls != 2 {
		t.Fatalf("fallback run: perItemCalls=%d; want 2 (one per page item)", perItemContent.perItemCalls)
	}
